...
refractive index $\eta$ of $1.4$. Note that \emph{every} medium in an LSQT
...
100,000. Enter the following three shell commands:
\begin{verbatim}
$ rm Substr.lsqt.lss
$ ../layered-sqt \
  -wi=200 \
...
$ ../layered-sqt-lssview Substr.lsqt.lss
\end{verbatim}
We first remove the existing LSS file with \texttt{rm}. This 
is necessary to regenerate sample directions---so long as this file exists, 
\texttt{layered-sqt} will ignore all flags except 
\texttt{-p/--path-count}, and add this number of paths to the existing 
simulation data. Then we run \texttt{layered-sqt} with flags \texttt{-wi=200},
\texttt{-p=100000}, and \texttt{-rp=0.2} (more on this flag in a moment).
...
which specifies the oversampling multiplier. 
For example, 
\begin{verbatim}
$ layered-sqt \
